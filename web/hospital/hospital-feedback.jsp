		...
		<div class="container">
			<div class="row justify-content-center pb-3">
				<div class="col-md-7 text-center heading-section heading-section-white ftco-animate ">
					<span class="subheading ">Read FeedBack</span>
					<h2 class="mb-4 text-dark">Our Patients Says</h2>
				</div>
			</div>
				...
				<div class="col-md-12">
					<div class="carousel-testimony owl-carousel ftco-owl">
                                          <%
                                            ...
                                            String s="SELECT * FROM hospital WHERE email='"+email+"'";
                                            ResultSet result=obj.displayData(s);
                                            if(result.next()){
                                                name=result.getString("name");
                                              }
                                            String query="SELECT * FROM hospital h JOIN feedback f on h.name=f.hname WHERE f.hname='"+name+"'";
                                            
                                            ResultSet rs=obj.displayData(query);
                                            
                                            while(rs.next()){
                                            val=rs.getString("useremail");
                                            que="SELECT * FROM user WHERE email='"+val+"'";
                                            pi=obj.displayData(que);
                                            if(pi.next()){
                                             Blob pic=pi.getBlob("upload_image");
                     ...
                     byte[] buffer = new byte[4096];
						...
						<div class="item">
							<div class="testimony-wrap text-center py-4 pb-5">
								...
								<div class="user-img">
                                                                    <img class="user-img" src='data:image/jpeg;base64,<%= Profilepic %>'height="50px",width="500px">
									<span class="quote d-flex align-items-center justify-content-center">
										<i class="icon-quote-left"></i>
									</span>
								</div>
								<div class="text px-4">
									<p class="name text-dark"><%= pi.getString("name")%></p>
									<span class="position text-dark"><%= rs.getString("treatments")%></span>
                                                                        <p class="name text-dark">
                                                                        
                                                   <%
                                                     int count = Integer.parseInt(rs.getString("rating"));
                                                        for (int i = 0; i < 5; i++) {
                                                            if (i < count) {
                                                    %>
									...
									<p class="mb-4 text-dark"><br><%= rs.getString("description")%></p>
