...
<%
     ...
     if(result.next()){
     String query="UPDATE hospital SET status='REJECTED' WHERE email=?";
     int reject=e1.Modify(query,b);
                if(reject>0){
                request.getSession();
                session.setAttribute("rejected", "Hospital Rejection Successful");
                response.sendRedirect("admin-managehospital.jsp");
                }else{
                request.getSession();
                session.setAttribute("failed", "Failed to Reject Hospital");
                response.sendRedirect("admin-managehospital.jsp");
                }
    }
