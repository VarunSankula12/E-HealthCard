...
<%
    ...
    String val="";String req="";
    String Profilepic="";
      String status="ACCEPTED";
        adminDao obj=new adminDao();
        String query="SELECT * FROM user u JOIN card c ON u.name = c.name JOIN renewdetails rd ON c.email = rd.email JOIN qr_code qc ON rd.email=qc.email";
        ResultSet rs=obj.displayData(query);
        while(rs.next()){
        req=rs.getString("renewReq");
            val=rs.getString("status");
            if(val.equalsIgnoreCase("ACCEPTED") && req.equalsIgnoreCase("Renew requested")){
...
%>
                                                ...
                                                <tr>
                                                    <td>
                                                        <img src='data:image/jpeg;base64,<%= Profilepic %>'height="100px",width="100px">
                                                    </td>
                                                    <td><%= rs.getString("ApplicationNum")%></td>
                                                    <th><%= rs.getString("name")%></th>
                                                    <td><%= rs.getString("email")%></td>
                                                    <td><%= rs.getString("towhom")%></td>
                                                    <td><%= rs.getString("planamount")%></td>
                                                    <td><%= rs.getString("years")%></td>
                                                    <td><%= rs.getString("expiry")%></td>
                                                    <td>Pending</td>
                                                    <td>
                                                         <a class="btn btn-success" href="admin-Acceptrenew.jsp?email=<%=java.net.URLEncoder.encode(rs.getString("email"),"UTF-8")%>&expiry=<%=java.net.URLEncoder.encode(rs.getString("expiry"),"UTF-8")%>">Renew</a> 
                                                         <a class="btn btn-danger" href="admin-Rejectrenew.jsp?email=<%=java.net.URLEncoder.encode(rs.getString("email"),"UTF-8")%>&expiry=<%=java.net.URLEncoder.encode(rs.getString("expiry"),"UTF-8")%>">Decline</a>
                                                    </td>
                                                </tr>
